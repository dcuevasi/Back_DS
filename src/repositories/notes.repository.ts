...
export interface NoteRepository {
  findAll(): Promise<NoteWithCategory[]>;
  findById(id: number): Promise<NoteWithCategory | null>;
  create(data: { title: string; content: string; categoryId: number }): Promise<NoteWithCategory>;
  update(id: number, data: { title?: string; content?: string; categoryId?: number }): Promise<NoteWithCategory>;
  remove(id: number): Promise<void>;
}

export const noteRepository: NoteRepository = {
  findAll() {
    return prisma.note.findMany({ include, orderBy: { createdAt: 'desc' } });
  },
